		...
		for (const m of menus) {
			if (m.type === 'a' && m.label && (m.label as string).toLowerCase().includes(value.toLowerCase())) {
				...
				for (const mm of m.items) {
					const match = mm.type === 'a' && mm.label && (mm.label as string).toLowerCase().includes(value.toLowerCase());
					if (match) {
...
run(document.getElementById('app')!, options, routes, (props: RouteSectionProps) => <InternalApp {...props} />);
