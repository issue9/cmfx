...
<APIDoc types={types} pkg="@cmfx/cdk" name="palette" />

<APIDoc types={types} pkg="@cmfx/cdk" name="nextPalette" />
...
通过 `@cmfx/cdk` 下的 `printElement` 打印内容的，可通过为子元素添加此类名以达到不打印的目的。
<APIDoc types={types} pkg="@cmfx/core" name="printElement" />
