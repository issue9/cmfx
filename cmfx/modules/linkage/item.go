...
func (r *Root[T]) Delete(id int64) error {
	...
	data := &modelLinkage{ID: id, Deleted: sql.NullTime{Time: time.Now()}}
	if _, err := r.linkage.DB().Update(data); err != nil {
		return err
	}
...
func (r *Root[T]) Update(id int64, val T) error {
	...
	item.Data = val
	_, err = r.linkage.DB().Update(&modelLinkage{
		ID:   id,
		Data: data,
	})
	if err != nil {
...
func (r *Root[T]) Add(parent int64, val ...T) error {
	...
	id, err := r.linkage.DB().LastInsertID(&modelLinkage{
		Data:   data,
		Key:    r.key,
		Parent: parent,
	})
