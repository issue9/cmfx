...
// 数据库的基本信息
type dbVO struct {
	Name               string        `json:"name" cbor:"name" yaml:"name"`                                           // 数据库驱动
	Version            string        `json:"version" cbor:"version" yaml:"version"`                                  // 数据库版本
	MaxOpenConnections int           `json:"maxOpenConnections" cbor:"maxOpenConnections" yaml:"maxOpenConnections"` // 连接数
	OpenConnections    int           `json:"openConnections" cbor:"openConnections" yaml:"openConnections"`          // 总的连接数
	InUse              int           `json:"inUse" cbor:"inUse" yaml:"inUse"`                                        // 当前的连接数
	Idle               int           `json:"idle" cbor:"idle" yaml:"idle"`                                           // 空闲的连接
	WaitCount          int64         `json:"waitCount" cbor:"waitCount" yaml:"waitCount"`                            // 等待的连接
	WaitDuration       time.Duration `json:"waitDuration" cbor:"waitDuration" yaml:"waitDuration" openapi:"string"`  // 新连接的平均等待时间
	MaxIdleClosed      int64         `json:"maxIdleClosed" cbor:"maxIdleClosed" yaml:"maxIdleClosed"`                // 最大空闲联接
	MaxLifetimeClosed  int64         `json:"maxLifetimeClosed" cbor:"maxLifetimeClosed" yaml:"maxLifetimeClosed"`    // 最生命周期联接

	// The total number of connections closed due to SetConnMaxIdleTime.
	MaxIdleTimeClosed int64 `json:"maxIdleTimeClosed,omitempty" cbor:"maxIdleTimeClosed,omitempty" yaml:"maxIdleTimeClosed,omitempty"`
}

// 系统信息
type infoVO struct {
	ID         string `json:"id" cbor:"id" yaml:"id"`                         // 应用名称
	Version    string `json:"version" cbor:"version" yaml:"version"`          // 应用版本号
	Uptime     string `json:"uptime" cbor:"uptime" yaml:"uptime"`             // 应用的上次启动时间
	Go         string `json:"go" cbor:"go" yaml:"go"`                         // 编译器的版本
	OS         *osVO  `json:"os" cbor:"os" yaml:"os"`                         // 系统信息
	Arch       string `json:"arch" cbor:"arch" yaml:"arch"`                   // CPU 架构
	CPUS       int    `json:"cpus" cbor:"cpus" yaml:"cpus"`                   // CPU 核心数量
	Goroutines int    `json:"goroutines" cbor:"goroutines" yaml:"goroutines"` // 当前运行的协程数量
	DB         *dbVO  `json:"db" cbor:"db" yaml:"db"`                         // 数据库的相关信息
}

type osVO struct {
	Platform string    `json:"platform" cbor:"platform" yaml:"platform"`
	Family   string    `json:"family" cbor:"family" yaml:"family"`
	Version  string    `json:"version" cbor:"version" yaml:"version"`
	Boot     time.Time `json:"boot" cbor:"boot" yaml:"boot"` // 系统的开机时间
}
...
type serviceVO struct {
	Title string `json:"title" cbor:"title" yaml:"title"`                         // 名称
	State state  `json:"state" cbor:"state" yaml:"state"`                         // 状态
	Err   string `json:"err,omitempty" cbor:"err,omitempty" yaml:"err,omitempty"` // 如果出错，表示错误内容，否则为空
}

// 计划任务
type jobVO struct {
	serviceVO `yaml:",inline"`
	Next      time.Time `json:"next,omitzero" cbor:"next,omitzero" yaml:"next,omitempty"` // 下一次执行时间
	Prev      time.Time `json:"prev,omitzero" cbor:"prev,omitzero" yaml:"prev,omitempty"` // 上一次执行时间
}

// 服务和计划任务
type servicesVO struct {
	Services []serviceVO `json:"services" cbor:"services" yaml:"services"` // 服务
	Jobs     []jobVO     `json:"jobs" cbor:"jobs" yaml:"jobs"`             // 计划任务
}
...
type problemVO struct {
	Prefix string `json:"prefix" cbor:"prefix" yaml:"prefix"` // URL 前缀以，如果此值不为空，与 ID 组成一上完整的地址。
	ID     string `json:"id" cbor:"id" yaml:"id"`             // 唯一 ID
	Status int    `json:"status" cbor:"status" yaml:"status"` // 对应的原始 HTTP 状态码
	Title  string `json:"title" cbor:"title" yaml:"title"`    // 错误的简要描述
	Detail string `json:"detail" cbor:"detail" yaml:"detail"` // 错误的明细
}
...
type backupListVO struct {
	Cron string          `json:"cron" cbor:"cron" yaml:"cron"`
	List []*backupFileVO `json:"list" cbor:"list" yaml:"list"`
}

type backupFileVO struct {
	Path string    `json:"path" cbor:"path" yaml:"path"`
	Size int       `json:"size" cbor:"size" yaml:"size"`
	Mod  time.Time `json:"mod" cbor:"mod" yaml:"mod"`
}
