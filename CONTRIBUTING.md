...
前端代码已经配置 `biome`，需要在 IDE 做相应的[配置](https://biomejs.dev/zh-cn/)，以开启自动检测功能。
...
- ssl: 对 `/apps/ssl` 下的内容进行了个性；
- server: 对 `/apps/server` 下的内容进行了个性；
...
- `make watch-dashboard` 热编译前端 admin 界面，之后可通过 `http://localhost:5173` 访问后台界面；
- `make watch -j2` 表示同时执行 `make watch-server` 和 `make watch-dashboard`；
...
```tsx
import {Button} from '@components/button/button' // 正确
import {Button} from '@components/button' // 不建议
```

- solid 的条件组件内尽量使用参数初始化，比如

```tsx
<Show when={props.xx}>{c=><div>c()</div>}</Show> // 正确
<Show when={props.xx}><div>{props.xx}</div></Show> // 不建议
```
...
前端文档采用 [tsdoc](https://tsdoc.org) 标准，与 JSDoc 稍有差异，比如 `@template` 应该改为 `@typeParam`，`@default` 应该改为 `@defaultValue` 等。
...
`apps/docs` 下的文档处理会稍微复制一些，分为以下几种类型：

 - 大段的文本内容是以 `.md` 格式保存的，根据语言的不同，可能的后缀名为 `.en.md` 或 `.zh-Hans.md` 等；
 - 各类 API 内容的翻译是从代码中提取的，按语言分别保存在 `api.en.json` 或是 `api.zh-Hans.json` 等文件中；
 - 小段的文本内容则和其它项目一样直接放在项目的 `messages` 目录下；
