...
export default function Timer(props: Props): JSX.Element {
	...
	timer = createMemo(() => {
		// 监视 props.duration 和 props.interval 的变化
		if (timer) {
			timer().stop();
		}
		...
		>
			<Show when={startField() >= getFieldIndex('days')!}>
				<div class={styles.item}>
					...
					<Show when={props.unit}>
						<span class={styles.unit}>{l.t('_c.timer.days')}</span>
					</Show>
				</div>
			...
			<Show when={startField() >= getFieldIndex('hours')!}>
				<div class={styles.item}>
					...
					<Show when={props.unit}>
						<span class={styles.unit}>{l.t('_c.timer.hours')}</span>
					</Show>
				</div>
				<div class={styles.sep}>{props.separator}</div>
			</Show>
			...
			<Show when={startField() >= getFieldIndex('minutes')!}>
				<div class={styles.item}>
					...
					<Show when={props.unit}>
						<span class={styles.unit}>{l.t('_c.timer.minutes')}</span>
					</Show>
				</div>
				<div class={styles.sep}>{props.separator}</div>
			</Show>
			...
			<Show when={startField() >= getFieldIndex('seconds')!}>
				<div class={styles.item}>
					...
					<Show when={props.unit}>
						<span class={styles.unit}>{l.t('_c.timer.seconds')}</span>
					</Show>
				</div>
			</Show>
		</div>
