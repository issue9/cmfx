...
export function buildRegion(): Array<Region> {
	...
	return Array.from(
		regions.entries().map(([id, timezones]) => ({
			id,
...
export function Root(props: Props): JSX.Element {
	...
	const localeRegions = createMemo(() => {
		return buildLocaleRegion(l.locale, l.displayStyle);
	});
	const tabs = regions.map(v => {
		return { id: v.id, label: v.displayName } as Tab.Item;
	});
	...
	const click = (value: string) => {
		...
		setSelected(value);
		if (value) {
			setTab(value.split('/')[0]);
		}

		if (props.onChange) {
			props.onChange(value, old);
		}
	};
