...
import { createMemo, createSignal, createUniqueId, type JSX, mergeProps, Show, splitProps } from 'solid-js';
...
interface DateProps extends Base {
	accessor: Form1.Accessor<Date | undefined, 'date'>;
}

interface NumberProps extends Base {
	accessor: Form1.Accessor<number | undefined, 'number'>;
}

interface StringProps extends Base {
	accessor: Form1.Accessor<string | undefined, 'string'>;
}
...
export function Root(props: Props): JSX.Element {
	if (isDateProps(props)) {
		...
		const accessor = Form1.fieldAccessor('accessor', val ? new Date(val) : undefined, 'date');
