...
export function Root(props: Props): JSX.Element {
	...
	return (
		...
		>
			...
			<Switch fallback={<Trigger style={Form1.fieldArea2Style(areas().inputArea)} />}>
				<Match when={props.onSearch}>
					<div style={Form1.fieldArea2Style(areas().inputArea)} class="w-full">
						<Dropdown.Root
							trigger="custom"
							items={candidate()}
							ref={el => {
			...
			<Show when={areas().countArea}>
				{area => (
					<div class={styles.count} style={Form1.fieldArea2Style(area())}>
						{count()}
					</div>
				)}
			</Show>
