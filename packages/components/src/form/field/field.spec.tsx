...
describe('fieldArea2Style', () => {
	test('pos', () => {
		expect(fieldArea2Style({ pos: 'top-start' })).toEqual({ 'grid-area': 'top-start' });
	});

	test('pos-cols', () => {
		expect(fieldArea2Style({ pos: 'top-start', cols: 2 })).toEqual({
			'grid-area': 'top-start',
	...
	test('pos-cols-rows', () => {
		expect(fieldArea2Style({ pos: 'top-start', cols: 2, rows: 3 })).toEqual({
			'grid-area': 'top-start',
			'grid-column-end': 'span 2',
			'grid-row-end': 'span 3',
		});
	...
	test('pos-rows', () => {
		expect(fieldArea2Style({ pos: 'top-start', rows: 3 })).toEqual({
			'grid-area': 'top-start',
			'grid-row-end': 'span 3',
		});
	});
...
describe('Field', async () => {
	const ct = await ComponentTester.build('Field', props => <Field {...props} />);
