 ...
 */
type ClickFunc = () => Promise<false | string | undefined>;
...
function buildRef(ref: HTMLDialogElement, l: Locale): Ref {
	return {
		...
		Action(title?: JSX.Element, click?: ClickFunc, def?: boolean): JSX.Element {
			...
			return (
				<Button.Root type={def ? 'submit' : 'button'} palette={def ? 'primary' : 'secondary'} onclick={btnClick}>
					{title}
				</Button.Root>
			);
		},
