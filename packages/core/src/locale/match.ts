...
/**
 * 从 available 中查找最符合 requested 的语言 ID
 *
 * @param requested - 匹配的值；
 * @param available - 被匹配的列表；
...
export function match(
    requested: string, available: Readonly<Array<string>>, preset: string, options: Options = {localeMatcher: 'lookup'}
): string {
    let ret = available.find(v => Intl.getCanonicalLocales(v)[0] === Intl.getCanonicalLocales(requested)[0]);
    if (ret) { return ret; }
