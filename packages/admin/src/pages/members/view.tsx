...
import { joinClass, Page, useLocale, useREST } from '@cmfx/components';
import { useParams } from '@solidjs/router';
import { type Component, createMemo, createSignal, For, type JSX, onMount, Show } from 'solid-js';

import { useOptions } from '@admin/app';
...
export function View(props: Props): JSX.Element {
	...
	onMount(async () => {
		const r = await rest.get<Member>(`/members/${id}`);
		if (!r.ok) {
		...
		<Page title="_p.member.view" class={joinClass(undefined, 'max-w-lg', styles.view)}>
			<div class={styles.info}>
				...
				<div class={styles.item}>
					<dl>
						<dt class="me-2">{l.t('_p.id')}</dt>
						<dd>{member().id}</dd>
					</dl>
					<dl>
						<dt class="me-2">{l.t('_p.no')}</dt>
						<dd>{member().no}</dd>
					</dl>
				</div>
				...
				<div class={styles.item}>
					<dl>
						<dt class="me-2">{l.t('_p.nickname')}</dt>
						<dd>{member().nickname}</dd>
					</dl>
					<dl>
						<dt class="me-2">{l.t('_p.sex')}</dt>
						<dd>{sexes().find(v => v.value === member().sex)?.label}</dd>
					</dl>
				</div>

				<div class={styles.item}>
					<dl>
						<dt class="me-2">{l.t('_p.state')}</dt>
						<dd>{states().find(v => v.value === member().state)?.label}</dd>
					</dl>
					<dl>
						...
						<dd class="flex gap-2">
							<For each={passports()}>
								{item => (
									<Show when={member().passports?.find(v => v.id === item.id)}>
