...
import { Choice, DataTable, InputText, Label, Page, useREST } from '@cmfx/components';
			...
			<DataTable
				...
				columns={
					[
						...
						{
							id: 'max',
							label: l.t('_p.system.max'),
							content: (_: string, val: Duration) => {
								formatDuration(l.durationFormat(), val);
							},
						},
						{
							id: 'min',
							label: l.t('_p.system.min'),
							content: (_: string, val: Duration) => {
						...
						{
							id: 'spend',
							label: l.t('_p.system.spend'),
							content: (_: string, val: Duration, route?: Route) => {
