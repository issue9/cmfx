...
export function Info(): JSX.Element {
	const rest = useREST();
	const l = useLocale();
	const bytesFormatter = createMemo(() => Formatter.createBytes(l));
	...
	return (
		<Page.Root title="_p.system.serverInfo" class={joinClass(undefined, styles.info)}>
			<fieldset class={joinClass(undefined, styles.panel, 'w-[45%]', '@max-2xl/info:w-full')}>
				...
				<dl>
					<dt>{l.t('_p.system.arch')}</dt>
					<dd>{info()?.arch}</dd>
				</dl>

				<dl>
					<dt>{l.t('_p.system.cpus')}</dt>
					<dd>{info()?.cpus}</dd>
				</dl>

				<dl>
					<dt>{l.t('_p.system.go')}</dt>
					<dd>{info()?.go}</dd>
				</dl>
				...
				<dl>
					...
					<dd>{info()?.os.platform}</dd>
				</dl>

				<dl>
					<dt>{l.t('_p.system.family')}</dt>
					<dd>{info()?.os.family}</dd>
				</dl>

				<dl>
					<dt>{l.t('_p.system.version')}</dt>
					<dd>{info()?.os.version}</dd>
				</dl>
				...
				<dl>
					<dt>{l.t('_p.system.waitCount')}</dt>
					<dd>{db()?.waitCount}</dd>
				</dl>

				<dl>
					<dt>{l.t('_p.system.waitDuration')}</dt>
					<dd>{db()?.waitDuration}</dd>
				</dl>
			</fieldset>
